...
# All Tcl 8.7+ interps can [load] thread 3.0.0
#
...
package ifneeded [string tolower thread] 3.0.0 \
    [list load [file join $dir tcl9thread300.dll] [string totitle thread]]
package ifneeded [string totitle thread] 3.0.0 \
    [list package require -exact [string tolower thread] 3.0.0]

# package ttrace uses some support machinery.

# In Tcl 8.7+ interps; use [::apply]

package ifneeded ttrace 3.0.0 [list ::apply {{dir} {
    ...
    } elseif {![catch {zipfs mount //zipfs:/lib/thread [file join $dir tcl9thread300.dll]}]} {
...
package ifneeded Ttrace 3.0.0 \
    [list package require -exact ttrace 3.0.0]
